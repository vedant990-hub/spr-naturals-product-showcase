...
export default function HomePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate network delay to look authentic
    setTimeout(() => {
              ...
              <Card className="p-8 bg-white border-sage-green/20 h-full">
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <Button 
                    ...
                    className="w-full bg-[#1b4332] hover:bg-[#1b4332]/90 text-white py-6 text-lg font-medium shadow-md transition-all hover:shadow-lg flex items-center justify-center gap-2"
                  >
                    {isSubmitting ? (
                      <span className="flex items-center gap-2">
                        <DotSpinner size="sm" color="white" /> Sending...
                      </span>
                    ) : (
                      <>
                  ...
                  {submitStatus.type === 'error' && (
                    <div className="mt-4 p-4 rounded-md bg-red-50/50 border border-red-100 text-center text-red-500">
                      <p className="text-sm mb-1">Sorry, there was an error sending your message. Please try again or contact us directly.</p>
                      <a href="tel:+917447755042" className="text-sm underline hover:text-red-700 transition-colors">
                        Call us: +91 7447755042
                      </a>
                    </div>
                  )}
                </form>
